...
public class ExcelUtility
{
    ...
    public ExcelUtility(string assetsPath)
    {
        try
        {
    ...
    public void ConvertToJson(string JsonPath, Encoding encoding)
    {
        ...
        for (int i = 3; i < rowCount; i++)
        {
            ...
            int voidCount = 0;//如果一行数据都是空的，break掉
            for (int j = 0; j < colCount; j++)
            {
                //读取第1行数据作为表头字段
                string field = mSheet.Rows[2][j].ToString();
                //Key-Value对应
                object o = mSheet.Rows[i][j].ToString();
                row[field] = o;
                if (o.ToString() == "") voidCount++;
            }
            if (voidCount == colCount)
            {
                break;
            }
            //添加到表数据中
    ...
    public void ConvertToCSV(string CSVPath, Encoding encoding)
    {
        ...
        for (int i = 0; i < rowCount; i++)
        {
            ...
            {
                //使用","分割每一个数值
                object o = mSheet.Rows[i][j];
                stringBuilder.Append(o + ",");
                if (o.ToString() == "") voidCount++;
            }
            if (voidCount == colCount)
            {
                stringBuilder.Remove(stringBuilder.Length - colCount, colCount);
                break;
            }
            //使用换行符分割每一行
    ...
    public void ConvertToXml(string XmlFile)
    {
        ...
        for (int i = 1; i < rowCount; i++)
        {
            ...
            for (int j = 0; j < colCount; j++)
            {
                string oStr = mSheet.Rows[i][j].ToString();
                string title = mSheet.Rows[0][j].ToString();
                tmpSb.Append("   <" + title + ">");
                tmpSb.Append(oStr);
                tmpSb.Append("</" + title + ">");
                tmpSb.Append("\r\n");
