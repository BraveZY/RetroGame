...
namespace UnityEngine.AssetBundles
{
    ...
    { 
            ...
            if(selectedNodes.Count == 1)
            {
                ...
                else
                {
                    ...
                    if (variant == null)
                    {
						...
						menu.AddItem (new GUIContent ("Split AssetBundle"), false, SplitBundle, selectedNodes);
                    }
                    else
                    {
		...
		void SplitBundle(object context)
		{
			var selectedNodes = context as List<AssetBundleModel.BundleTreeItem>;
			...
			{
				if (selectedNodes.Count == 1) {
					string fullBundlePath = selectedNodes [0].bundle.m_Name.bundleName;
					if (EditorUtility.DisplayDialog ("Split", "确定要拆分" + fullBundlePath + "内容吗？", "确定", "取消")) {
						var list = AssetDatabase.GetAssetPathsFromAssetBundle (fullBundlePath);
						AssetDatabase.RemoveAssetBundleName (fullBundlePath,true);
						foreach (var item in list) {
							
							AssetImporter aip = AssetImporter.GetAtPath (item);
							var fileNameWithoutExtension = Path.GetFileNameWithoutExtension (item);
							aip.assetBundleName = fullBundlePath +"/"+ fileNameWithoutExtension;
						}
