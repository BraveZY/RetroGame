...
涉及反应时间、任务限时、擦除阈值、擦错容错、连击倍率、目标尺寸或输入平滑参数时，必须在 `game-design/blackboard-eraser/balance/<feature-slug>/` 创建或更新调参记录。
...
涉及玩法规则、体感输入、板擦映射、黑板内容、UI、场景、Prefab、资源、存档、设备或测试 seam 的任务，在进入实现前应在 `game-design/blackboard-eraser/system-design/<feature-slug>/` 创建或更新系统框架设计文档。
