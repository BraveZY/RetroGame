...
├── CoreGameScript/
│   ├── <Game>_Script/                  # 新游戏代码的默认物理根目录
│   │   ├── Runtime/                    # 游戏流程、玩法、状态机
│   │   ├── Input/                      # Pose/手势到玩法的适配，不直接复制 PoseAPI
│   │   ├── UI/
│   │   ├── Data/
│   │   ├── Config/
│   │   ├── Tests/                      # 独立 Test asmdef，仅 Editor/测试使用
│   │   └── Assembly<Game>.asmdef       # 游戏专属运行时程序集
│   ├── AssemblyUpdateScriptsHOT/       # 已有公共 HOT 程序集；不作为新游戏默认物理根
│   └── AssemblyUpdateScriptsAOT/       # 启动/平台桥接等必须随包代码
└── ...
```

B 的 `Basketball_Assets + Basketball_Script`、`Tennis_Assets + Tennis_Script`、`Ski_Assets + Ski_Script` 是当前应遵循的**实际物理放置**：资产与代码一一成对，且游戏测试随 `<Game>_Script/Tests/` 归属。独立程序集是否能热更新是第二个问题：保龄球 `Bowing_Script/AssemblyBowling.asmdef` 未进入 B 的 HybridCLR 热更新清单，说明“位于 `<Game>_Script`”不等于“已经 HOT 发布”。历史目录存在 `Scene/Scenes`、`prefabs/Prefabs` 等大小写不一致；**新目录统一按本模板命名**，不要再扩散历史写法。
...
### 3.1 默认物理放置：游戏专属脚本根；HOT 资格单独配置

新游戏 C# 默认放在参考项目已经验证的物理位置：

```text
Assets/CoreGameScript/<Game>_Script/
```

根目录放置 `Assembly<Game>.asmdef`，并可继续细分 `Runtime/`、`Input/`、`UI/`、`Data/`、`Config/` 与 `Tests/`。测试必须使用单独的 Test asmdef（例如 `<Game>.Tests`），引用游戏程序集，且只在 Editor/测试环境编译。
...
Unity 的编译边界由 `.asmdef` 决定，不由普通文件夹决定。当前 A 的 `ProjectSettings/HybridCLRSettings.asset` 只配置了 `AssemblyUpdateScriptsHOT`；因此新建 `Assembly<Game>` 后，在完成 3.2 的 HybridCLR、清单和构建产物同步前，它只是一个**已分组的运行时程序集**，不能宣称已经进入 HOT DLL。只有明确要继续维护旧公共程序集时，才把代码放到 `AssemblyUpdateScriptsHOT/`；这属于公共程序集维护，不是新游戏默认落点。

| 放入位置 | 适合内容 | 不适合内容 |
| --- | --- | --- |
| `<Game>_Script/Assembly<Game>.asmdef`（默认物理放置） | 新游戏玩法、UI、状态、输入适配；与 `<Game>_Assets` 成对维护和回滚 | 未完成 3.2 前，不得声称它已被 HOT 发布。 |
| `AssemblyUpdateScriptsHOT/`（已有公共程序集） | 已确认需要与公共 HOT 逻辑共同发布的代码 | 不应用作新游戏默认目录；不要混入游戏私有测试或 Editor 代码。 |
...
PoseAPI、GameCore、通用 UI/工具
        ↑
Assembly<Game>（位于 <Game>_Script/；完成发布链后才是独立 HOT 玩法代码）
        ↑
<Game>_Assets（Prefab / Scene 上挂载对应玩法组件）
```

- 游戏 A 不应引用游戏 B 的程序集或私有 Prefab；抽出的共性先经过公共模块评审。
- `Editor`、`Tests` 随 `<Game>_Script/` 归属但使用独立 asmdef，不能进入运行时 DLL 或热更新 DLL。
...
1. 创建 `<Game>_Assets` 与 `<Game>_Script`，先确定游戏标识、主场景名和运行时程序集名。
2. 导入资源到游戏私有目录，设置 AssetBundle 分组；创建 `<Game>_Main` 场景和 Prefab。
3. 在 `<Game>_Script` 内完成玩法、UI、输入适配和配置；创建 `Assembly<Game>.asmdef`，测试放入同根 `Tests/` 并使用独立 Test asmdef。
...
- [ ] 游戏代码在 `CoreGameScript/<Game>_Script`，资源在对应 `<Game>_Assets`，且运行时 asmdef、Editor/Test asmdef 的边界清晰。
...
1. 不因为 `<Game>_Script/Assembly<Game>.asmdef` 已存在，就误以为它会自动热更；必须完成 3.2 的 HybridCLR 设置、`AssemblyHotList.txt`、DLL 产物和发布清单核对。
...
- 游戏标识：<Game>
- 主场景：<Game>_Main
- 游戏代码目录：CoreGameScript/<Game>_Script
- 运行时程序集：Assembly<Game>
- HOT 发布接入：待接入 / 已接入（已接入时必须写明 HybridCLR 配置、AssemblyHotList 条目和 DLL 产物）
- AssetBundle 分组：<待填写>
- 入口菜单/加载方式：<待填写>
